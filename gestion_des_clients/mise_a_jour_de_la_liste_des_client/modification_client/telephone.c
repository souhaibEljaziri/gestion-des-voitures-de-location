...
liste_clt Modify_element(liste_clt l,char x[20])
{
    liste_clt p=l,q=NULL;
    ...
    while(p!=NULL)
    {
        ...
        if((p!=NULL)&&(strcmp(p->info2.cin,x)==0))
        {
            do
            {
            system("cls");
           ...
           gotoxy(58,2);printf("saisir le telephone de client:");
           gotoxy(58,5);scanf("%s",&(p->info2.tel));
            }
            while((strlen(p->info2.tel)!=8)||(verif_cin(p->info2.tel)==0));
            return l;
        }
...
system("color 9a");
 ...
 system("type creation_voiture\\pose.txt");
    gotoxy(58,2);printf("saisir le cin de client:");
     gotoxy(58,5);printf("00000000");
    gotoxy(58,5);gets(x);
    }
while((test(l,x)!=0));
l=Modify_element(l,x);
 enregistrement(l,f);
