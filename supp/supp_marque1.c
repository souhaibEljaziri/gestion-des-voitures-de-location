...
if(p!=NULL)
{
    printf("\nil y'a des voiture dans la liste -_- !!! ----menu----> !!! MISE A JOUR DE LA LISTE DES VOITURE{si tu vue ajouter des voiture}\n");
}
...
liste_voit sup_marque(liste_voit l)
{
    int rep,t;
liste_voit p,r;char marque;
int ok;
do{
...
p=l;
        t=taille(p);
            ...
            scanf("%d",&marque);
        if(strcmp(p->info1.marque,marque))
    {
        r=p;
        p=p->suiv1;
        free(r);
        ...
        else
       {
           ...
           while(((ok=strcmp((p->suiv1)->info1.marque,marque)==0))&&((p->suiv1)!=NULL))
           {
              p=p->suiv1;
           }
           if(ok==1)
...
affiche(l);
        ...
        do
        {
            printf("Voulez vous supprimer un autre etudiant :");
            scanf("%d",&rep);
        }while(!(rep==1 || rep==0));
    }
    while(!(rep==0));
